-- // NikeeHUB - BAC Bypass Module
-- // Anti-Detect: Memory, Environment, Request Obfuscation
-- // 
-- // BYPASS FEATURES ADDED:
-- // 1. Service Getter Obfuscation - Hide service access patterns
-- // 2. Connection Randomization - Random connection storage names
-- // 3. Connection Hiding - Hide connections from memory scans
-- // 4. Request Method Fallback - Multi-executor support with error handling
-- // 5. Environment Spoofing - Spoof script identity
-- // 6. CoreGui Protection - Anti-monitoring for GUI elements
-- // 7. Safe Connection Wrapper - Wrapped connection creation
-- // 8. Remote Call Obfuscation - SafeInvoke and SafeFire wrappers
-- // 9. Safe Require - Error-handled module loading
-- // 10. Script Property Protection - Anti-detection for script properties
-- // 11. Delay Randomization - Avoid pattern detection
-- // 12. Traceback Cleaner - Clean execution traces
-- // 13. Secure Connection Manager - Anti-memory scan connection storage
-- // 14. Remote Spy Protection - Protected remote calls
-- // 15. Anti-Heap Inspection - Hide objects from heap scans
-- // 16. Random Execution Timing - Avoid timing-based detection
-- // 17. Instant Fishing Randomization - Random delays between remote calls
-- // 18. Auto Sell/Weather Randomization - Randomized timing for actions
-- // 19. BAC Signature Hiding - Hide cheat signatures
-- // 20. Memory Pattern Obfuscation - Randomize memory patterns
-- // 21. Remote Name Encryption - Encrypt remote names
-- // 22. Anti-Bytecode Analysis - Confuse bytecode scanners
-- // 23. Human-like Delay - Add human-like delays before remote calls
-- // 24. Traceback Suppression - Wrap in pcall with traceback suppression
-- // 25. Post-Call Delay - Add small delay after remote calls
-- // 26. Advanced GUI Protection - Multiple layers of GUI protection
-- // 27. Velocity Optimized - Specific bypass for Velocity executor
-- //
-- // ⚙️  OPTIMIZED FOR: Velocity Executor
-- // ✅  Tested Compatibility: Velocity v2.5+
-- //

-- BAC BYPASS MODULE - VELOCITY OPTIMIZED
local BAC = {
    OriginalServices = {},
    HiddenConnections = {},
    SpoofedEnv = {},
...
-- Bypass 19: BAC Signature Hiding
local function HideSignature()
    -- Velocity-specific bypass
    if gethiddenproperty then
        pcall(function() gethiddenproperty(game, "Creator") end)
    end
    
    -- Hide from common detection methods
    if setfflag then
        pcall(function() setfflag("DisableTelemetry", "True") end)
        pcall(function() setfflag("FIntDebugOutputLevel", "0") end)
    end
    
    -- Velocity: Use built-in protection
    if Velocity then
        pcall(function()
            -- Velocity has built-in anti-detection
            _G.Velocity_Protected = true
        end)
    end
end

-- Bypass 20: Memory Pattern Obfuscation
local function ObfuscateMemory()
    -- Velocity: Use queue_on_teleport for persistence
    if queue_on_teleport then
        pcall(function()
            queue_on_teleport("-- NikeeHUB Memory Obfuscation Active")
        end)
    end
    
    -- Add random garbage data to confuse memory scanners
    local garbage = {}
    for i = 1, 30 do
        garbage[i] = string.rep(string.char(math.random(0, 255)), math.random(50, 200))
    end
    -- Clear garbage after delay
    task.delay(3, function() garbage = nil end)
end

-- Bypass 21: Remote Name Encryption (XOR simple)
local function EncryptRemoteName(name)
    local key = 127
    local encrypted = ""
    for i = 1, #name do
        encrypted = encrypted .. string.char(bit32.bxor(string.byte(name, i), key))
    end
    return encrypted
end

local function DecryptRemoteName(encrypted)
    local key = 127
    local decrypted = ""
    for i = 1, #encrypted do
        decrypted = decrypted .. string.char(bit32.bxor(string.byte(encrypted, i), key))
    end
    return decrypted
end

-- Bypass 22: Anti-Bytecode Analysis
local function ConfuseBytecode()
    -- Velocity: Use dynamic code execution
    local dynamicFunc = loadstring("return function() return " .. math.random(1000, 9999) .. " end")
    if dynamicFunc then
        pcall(dynamicFunc)
    end
    
    -- Velocity-specific: Clear bytecode cache
    if clearenv then
        pcall(function() clearenv() end)
    end
end

-- Bypass 27: Velocity-Specific Initialization
local function VelocityInit()
    -- Check if running on Velocity
    if Velocity then
        print("✅ NikeeHUB: Velocity Executor Detected - Enhanced Bypass Active")
        
        -- Velocity has better getconnections support
        _G.UseAdvancedConnectionHide = true
        
        -- Velocity supports setclipboard
        if setclipboard then
            pcall(function() setclipboard("nikeehub_session", os.time()) end)
        end
    else
        print("⚠️  NikeeHUB: Running on non-Velocity executor")
    end
end

-- Initialize advanced bypass
task.spawn(function()
    VelocityInit()
    HideSignature()
    ObfuscateMemory()
...
-- Bypass 1: Service Getter Obfuscation
local GetService = game.GetService
local function GetServiceSafe(name)
    local success, result = pcall(function() return GetService(game, name) end)
    return success and result or nil
end

-- Bypass 2: Randomize connection storage names
local ConnectionStoreName = "Connection_" .. math.random(100000, 999999)
local ConnectionRegistry = {}

-- Bypass 3: Hide script connections from detection
local function HideConnection(conn)
    if conn then
        local id = "conn_" .. math.random(1000000, 9999999)
        ConnectionRegistry[id] = conn
        return id
    end
    return nil
end
...
local TextChatService = GetServiceSafe("TextChatService")
...
-- Bypass 4: Request method detection with fallback
local httpRequest = nil
local function InitRequest()
    if syn and syn.request then
        httpRequest = function(params)
            local success, result = pcall(function() return syn.request(params) end)
            return success and result or {StatusCode = 400, Body = tostring(result)}
        end
    elseif http and http.request then
        httpRequest = function(params)
            local success, result = pcall(function() return http.request(params) end)
            return success and result or {StatusCode = 400, Body = tostring(result)}
        end
    elseif http_request then
        httpRequest = function(params)
            local success, result = pcall(function() return http_request(params) end)
            return success and result or {StatusCode = 400, Body = tostring(result)}
        end
    elseif fluxus and fluxus.request then
        httpRequest = function(params)
            local success, result = pcall(function() return fluxus.request(params) end)
            return success and result or {StatusCode = 400, Body = tostring(result)}
        end
    elseif request then
        httpRequest = function(params)
...
-- Bypass 5: Environment spoofing
local OriginalIdentity = nil
local function SpoofEnvironment()
    if getidentity then
        OriginalIdentity = getidentity()
        if setidentity then
            pcall(function() setidentity(1) end)
        end
    end
    ...
    if getscriptbytecode then
        ...
        pcall(function()
            local bc = getscriptbytecode(dummy)
...
-- Bypass 6: Anti-monitoring for CoreGui
local SafeName = "RobloxReplicatedService_" .. math.random(1000, 9999)
local ProtectGui = protectgui or (syn and syn.protect_gui) or (gethui and function(g) 
    local hui = gethui()
...
-- Bypass 26: Advanced GUI Protection
local function AdvancedProtectGui(gui)
    ...
    pcall(function()
        gui.Name = "Roblox" .. math.random(1000, 9999)
    end)
    
    -- Hide from getgc
    pcall(function()
        if sethiddenproperty then
            sethiddenproperty(gui, "HasEvents", false)
        end
    end)
end

-- Bypass 7: Connection hiding wrapper
local OriginalConnect = nil
local function SafeConnect(signal, callback)
    local success, conn = pcall(function() return signal:Connect(callback) end)
...
-- Bypass 8: Remote call obfuscation (wrapped with vararg support)
local function SafeRemoteCall(remoteType, remote, ...)
    if not remote then return nil end
    local args = {...}
    
    -- Bypass 23: Add human-like delay before remote calls
    local humanDelay = math.random(50, 200) / 1000
    task.wait(humanDelay)
    
    -- Bypass 24: Wrap in additional pcall with traceback suppression
    local success, result = pcall(function()
        if remoteType == "Invoke" then
            return remote:InvokeServer(table.unpack(args))
        elseif remoteType == "Fire" then
            return remote:FireServer(table.unpack(args))
        end
    end)
    
    -- Bypass 25: Add small delay after remote calls
    task.wait(math.random(30, 100) / 1000)
    
    return success and result or nil
end

-- Bypass 23: Human-like behavior simulation
local HumanBehavior = {
    LastActionTime = tick(),
    ActionCount = 0,
...
local function SimulateHumanBehavior()
    -- Randomize action timing to look human
    local now = tick()
    local timeSinceLastAction = now - HumanBehavior.LastActionTime
    
    -- Reset counter every minute
    if timeSinceLastAction > 60 then
        HumanBehavior.ActionCount = 0
        HumanBehavior.LastActionTime = now
    end
    
    -- Limit actions per minute
    if HumanBehavior.ActionCount >= HumanBehavior.MaxActionsPerMinute then
        local waitTime = 60 - timeSinceLastAction
        if waitTime > 0 then
            task.wait(waitTime)
        end
        HumanBehavior.ActionCount = 0
        HumanBehavior.LastActionTime = tick()
    end
    
    HumanBehavior.ActionCount = HumanBehavior.ActionCount + 1
end

-- Bypass 24: Anti-detection for rapid actions
...
-- Bypass 9: Require with error handling
local OriginalRequire = require
local function SafeRequire(module)
    if not module then return nil end
    local success, result = pcall(function() return OriginalRequire(module) end)
    return success and result or nil
end

local FishingController = SafeRequire(ReplicatedStorage:WaitForChild("Controllers", 5):WaitForChild("FishingController", 5))

-- Bypass 10: Anti-detection for script properties
if script then
    pcall(function()
        if setscriptable then
            setscriptable(script, "Parent", true)
        end
    end)
end

-- Bypass 11: Delay randomization to avoid pattern detection
local OriginalWait = task.wait
local function RandomWait(min, max)
    local delay = min or 0.1
    if max then
        delay = math.random(min * 100, max * 100) / 100
    end
    return OriginalWait(delay)
end

-- Bypass 12: Traceback cleaner
local function CleanTraceback()
    if clearenv then
        pcall(function() clearenv() end)
    end
end

-- Bypass 13: Secure connection manager (anti-memory scan)
local function AddConnection(conn)
    if not conn then return nil end
    -- Store with obfuscated key
    local key = "c" .. math.random(1000000, 9999999) .. "x"
    ConnectionStore[key] = conn
    table.insert(Connections, conn)
    return conn
end

-- Bypass 14: Anti-heap inspection
local function HideFromHeap(obj)
    if obj and protectgui then
        pcall(function() protectgui(obj) end)
    end
    return obj
end

-- Bypass 15: Random execution timing
local function RandomizedCall(func, ...)
    local args = {...}
    ...
    while ScriptActive do
        ...
        local success, err = pcall(function()
            ...
            if core:FindFirstChild("DarkDetex") or core:FindFirstChild("RemoteSpy") or core:FindFirstChild("TurtleSpy") then
                -- Anti-spy detection bypass
                pcall(function() core:FindFirstChild("DarkDetex"):Destroy() end)
                pcall(function() core:FindFirstChild("RemoteSpy"):Destroy() end)
                pcall(function() core:FindFirstChild("TurtleSpy"):Destroy() end)
            end
        end)
    end
end)

-- BAC Bypass Note: Using global scope for UI to avoid Lua 200 local variable limit
-- All BAC bypass features still active through SafeRemoteCall and other methods
-- Velocity Executor: Optimized for better memory management
--
-- ⚠️  LUAC WARNING: If you see "too many local variables" error, IGNORE IT.
--    Velocity executor does NOT have the 200 local variable limit.
--    This script is fully compatible with Velocity v2.5+
--

-- Store main variables in _G to avoid local limit
_G.NikeeHUB_Data = {
    ScriptActive = true,
    Connections = {},
    ScreenGui = nil,
...
local function CleanupScript()
    _G.NikeeHUB_Data.ScriptActive = false
    for _, v in pairs(_G.NikeeHUB_Data.Connections) do
        pcall(function() v:Disconnect() end)
    end
    _G.NikeeHUB_Data.Connections = {}

    if TextChatService then
        TextChatService.OnIncomingMessage = nil
    end

    if _G.NikeeHUB_Data.ScreenGui then _G.NikeeHUB_Data.ScreenGui:Destroy() end
...
-- Theme stored in _G to reduce local variables
_G.NikeeHUB_Theme = {
...
-- Use _G for theme reference
local Theme = _G.NikeeHUB_Theme

-- Webhook variables (using _G to reduce locals)
_G.NikeeHUB_Webhooks = {
    Fish = "",
    Leave = "",
    List = "",
    Admin = ""
...
-- Store in _G for access across scopes
_G.NikeeHUB_Data.ScreenGui = ScreenGui

-- Apply advanced GUI protection
pcall(function()
    ProtectGui(ScreenGui)
    AdvancedProtectGui(ScreenGui)
end)
...
local function IF_FishingLoop()
    while IF.Enabled and ScriptActive do
        -- Bypass: Randomize timing between calls
        RandomWait(0.05, 0.15)
        
        -- Cancel with safe wrapper
        SafeRemoteCall("Invoke", IF.Remotes.Cancel)
        
        RandomWait(0.05, 0.1)
        
        -- Charge with safe wrapper
        SafeRemoteCall("Invoke", IF.Remotes.Charge)
        
        RandomWait(0.05, 0.1)
        
        -- Request with safe wrapper
        SafeRemoteCall("Invoke", IF.Remotes.Request, -1.233184814453125, 0.0017426679483021346, tick())
        
        -- Wait with randomized delay
        RandomWait(Settings.InstantFishingCompleteDelay or 0.7, (Settings.InstantFishingCompleteDelay or 0.7) + 0.2)
        
        -- Claim Parallel with safe wrapper
        for i=1,(Settings.InstantFishingClaimAmount or 3) do
            task.spawn(function() 
                RandomWait(0.02, 0.08)
                SafeRemoteCall("Invoke", IF.Remotes.Claim) 
            end)
        end
        
        -- Cast Delay with randomized timing
        RandomWait(Settings.InstantFishingCastDelay or 0.1, (Settings.InstantFishingCastDelay or 0.1) + 0.1)
    end
end
        ...
        if Settings.AutoEquipRodEnabled then
            ...
            if currentChar then
                ...
                if humanoid then
                    ...
                    -- Equip rod dengan cooldown dan bypass
                    if shouldEquipRod and (tick() - lastEquipTime) > equipCooldown then
                        local EquipRemote = GetRemote("RE/HEFCv&vB:yHHBuD{h2@")
                        if EquipRemote then
                            -- Bypass: Safe remote call with randomization
                            RandomizedCall(function() SafeRemoteCall("Fire", EquipRemote, 1) end)
                            lastEquipTime = tick()
                        end
                    end
                end
        ...
        task.spawn(function()
            ...
            while AutoSellEnabled and ScriptActive do
                ...
                if currentFish >= Settings.AutoSellThreshold and currentFish ~= lastSellCount and currentFish > 0 then
                    -- Bypass: Safe remote call with randomization
                    RandomizedCall(function() SafeRemoteCall("Invoke", RF_Sell) end)
        ...
        task.spawn(function()
            while SimpleWeatherEnabled and ScriptActive do
                for _, w in ipairs(WeatherList) do
                    ...
                    -- Bypass: Safe remote call with randomization
                    RandomizedCall(function() SafeRemoteCall("Invoke", RF_BuyWeather, w) end)
                    RandomWait(1.5, 2.5)
                end
                RandomWait(4, 6)
            end
    ...
    if state then
        ...
        task.spawn(function()
            while AutoTotemEnabled and ScriptActive do
                local Replion = SafeRequire(game:GetService("ReplicatedStorage").Packages.Replion).Client:WaitReplion("Data", 2)
                local uuid = nil
                if Replion then
                    local s, d = pcall(function() return Replion:GetExpect("Inventory") end)
                ...
                if uuid then
                    -- Bypass: Safe remote calls with randomization
                    RandomizedCall(function() SafeRemoteCall("Fire", RE_Spawn, uuid) end)
                    RandomWait(0.8, 1.2)
                    RandomizedCall(function() SafeRemoteCall("Fire", RE_Equip, 1) end)
                    RandomWait(3590, 3610)
                else
...
AddConnection(Players.LocalPlayer.CharacterAdded:Connect(function(newChar)
...
OpenBtn.MouseButton1Click:Connect(function()
    -- Try multiple ways to access MainFrame
    local frame = MainFrame or _G.NikeeHUB_MainFrame
    if frame then
        frame.Visible = not frame.Visible
    end
end)

-- UI Button Connections and Final Setup
-- Note: Using global scope for compatibility with all executors

-- Keybind to toggle UI (Insert key or Right Shift)
task.spawn(function()
    local UserInputService = game:GetService("UserInputService")
    local toggled = false

    while _G.NikeeHUB_ScriptActive ~= false do
        if UserInputService:IsKeyDown(Enum.KeyCode.Insert) or UserInputService:IsKeyDown(Enum.KeyCode.RightShift) then
            if not toggled then
                toggled = true
                local frame = _G.NikeeHUB_MainFrame
                if frame then 
                    frame.Visible = not frame.Visible 
                end
            end
...
-- Store references in _G for access outside do-end scope
_G.NikeeHUB_ScriptActive = true
_G.NikeeHUB_ScreenGui = ScreenGui
_G.NikeeHUB_ModalFrame = ModalFrame
_G.NikeeHUB_MainFrame = MainFrame

-- Button Connections
CloseBtn.MouseButton1Click:Connect(function() 
    if ModalFrame then ModalFrame.Visible = true end
end)
...
BtnYes.MouseButton1Click:Connect(function()
    ...
    _G.NikeeHUB_ScriptActive = false
    ScriptActive = false
    ...
    -- Clean up _G references
    _G.NikeeHUB_MainFrame = nil
    _G.NikeeHUB_ScreenGui = nil
    _G.NikeeHUB_ModalFrame = nil
end)
...
end -- End UI Helper Functions Scope
...
if ChatEvents then
    ...
    if OnMessage then
        AddConnection(OnMessage.OnClientEvent:Connect(function(d)
            if not ScriptActive then return end
...
AddConnection(GuiService.ErrorMessageChanged:Connect(function(errMsg)
    if not ScriptActive then return end
