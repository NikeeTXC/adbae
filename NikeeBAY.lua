-- ============================================
-- NikeeBAY - Fishing Webhook Script
-- UI: Using MainUi.lua Library (UI/MainUi.lua)
-- ============================================
...
if CoreGui:FindFirstChild("RobloxReplicatedService") then CoreGui:FindFirstChild("RobloxReplicatedService"):Destroy() end
if CoreGui:FindFirstChild("XAL_System") then CoreGui:FindFirstChild("XAL_System"):Destroy() end
...
function Chloex:Window(GuiConfig)
    ...
    GuiConfig.Title = GuiConfig.Title or "NikeeBAY"
    ...
    Close.Activated:Connect(function()
        ...
        Title.Font = Enum.Font.GothamBold; Title.Text = "NikeeBAY Window"; Title.TextSize = 22; Title.TextColor3 = Color3.fromRGB(255, 255, 255); Title.ZIndex = 52; Title.Parent = Dialog
    ...
    function Tabs:AddTab(TabConfig)
        ...
        function Sections:AddSection(Title, AlwaysOpen)
            ...
            function Items:AddDropdown(DropdownConfig)
                DropdownConfig = DropdownConfig or {}; DropdownConfig.Title = DropdownConfig.Title or "Title"
                DropdownConfig.Content = DropdownConfig.Content or ""; DropdownConfig.Options = DropdownConfig.Options or {}
                DropdownConfig.Default = DropdownConfig.Default or (DropdownConfig.Options and DropdownConfig.Options[1]) or "None"
                DropdownConfig.Callback = DropdownConfig.Callback or function() end
                local Dropdown = Instance.new("Frame"); Dropdown.BackgroundColor3 = Color3.fromRGB(255, 255, 255); Dropdown.BackgroundTransparency = 0.935
                Dropdown.BorderColor3 = Color3.fromRGB(0, 0, 0); Dropdown.BorderSizePixel = 0; Dropdown.LayoutOrder = CountItem
                Dropdown.Size = UDim2.new(1, 0, 0, 46); Dropdown.Name = "Dropdown"; Dropdown.Parent = SectionAdd
                Instance.new("UICorner", Dropdown).CornerRadius = UDim.new(0, 4)
                local DropdownTitle = Instance.new("TextLabel"); DropdownTitle.Font = Enum.Font.GothamBold; DropdownTitle.Text = DropdownConfig.Title
                DropdownTitle.TextColor3 = Color3.fromRGB(230, 230, 230); DropdownTitle.TextSize = 13; DropdownTitle.TextXAlignment = Enum.TextXAlignment.Left
                DropdownTitle.BackgroundTransparency = 1; DropdownTitle.Position = UDim2.new(0, 10, 0, 10); DropdownTitle.Size = UDim2.new(1, -180, 0, 13); DropdownTitle.Parent = Dropdown
                local DropdownBtn = Instance.new("TextButton"); DropdownBtn.BackgroundColor3 = Color3.fromRGB(255, 255, 255); DropdownBtn.BackgroundTransparency = 0.95
                DropdownBtn.Position = UDim2.new(1, -7, 0.5, 0); DropdownBtn.Size = UDim2.new(0, 148, 0, 30); DropdownBtn.Text = DropdownConfig.Default .. " ▼"
                DropdownBtn.TextColor3 = Color3.fromRGB(255, 255, 255); DropdownBtn.TextSize = 12; DropdownBtn.Parent = Dropdown
                Instance.new("UICorner", DropdownBtn).CornerRadius = UDim.new(0, 4)
                local DropdownFunc = { Value = DropdownConfig.Default }
                DropdownBtn.MouseButton1Click:Connect(function()
                    local currentVal = DropdownFunc.Value
                ...
                function DropdownFunc:Set(val) DropdownFunc.Value = val; DropdownBtn.Text = val .. " ▼" end
...
local Window = Chloex:Window({ Title = "NikeeBAY", Footer = "ITG Webhook", Color = Color3.fromRGB(0, 139, 139), Image = "rbxassetid://108886429866687" })

local SessionStatsTab = Window:AddTab({ Name = "Server Info", Icon = "stat" })
local SessionStatsSection = SessionStatsTab:AddSection("Session Statistics", true)
local FhisingTab = Window:AddTab({ Name = "Fhising", Icon = "fish" })
local FhisingSection = FhisingTab:AddSection("Fishing Features", true)
...
local WebhookTab = Window:AddTab({ Name = "Notification", Icon = "web" })
local WebhookSection = WebhookTab:AddSection("Notification Settings", true)
local WebhookURLSection = WebhookTab:AddSection("Webhook URLs", false)
local AdminBoostTab = Window:AddTab({ Name = "Admin Boost", Icon = "boss" })
local AdminBoostSection = AdminBoostTab:AddSection("Admin Features", true)
local TagTab = Window:AddTab({ Name = "List Player", Icon = "player" })
local TagSection = TagTab:AddSection("Player List", true)
local SettingTab = Window:AddTab({ Name = "Setting", Icon = "settings" })
local SettingSection = SettingTab:AddSection("General Settings", true)
local SaveConfigTab = Window:AddTab({ Name = "Save Config", Icon = "bag" })
local SaveConfigSection = SaveConfigTab:AddSection("Configuration", true)

print("✅ ITG System UI Loaded!")
...
local function getFishCount()
    local playerGui = Players.LocalPlayer:WaitForChild("PlayerGui", 5); if not playerGui then return 0 end
    ...
    if inv then
        ...
        if label then return tonumber((label.Text or "0/???"):match("(%d+)/")) or 0 end
    end
    return 0
end
...
local function SendWebhook(data, category)
    ...
    if category == "SECRET" then
        ...
        local lines = { "⚓ Fish: " .. data.Item }; if data.Mutation and data.Mutation ~= "None" then table.insert(lines, "🧬 Mutation: " .. data.Mutation) end
    ...
    elseif category == "STONE" then
        ...
        local lines = { "💎 Stone: " .. data.Item }; if data.Mutation and data.Mutation ~= "None" then table.insert(lines, "✨ Mutation: " .. data.Mutation) end
        table.insert(lines, "⚖️ Weight: " .. data.Weight); descriptionText = "Player: " .. pName .. "\n\n```\n" .. table.concat(lines, "\n") .. "\n```"
    ...
    pcall(function() httpRequest({ Url = TargetURL, Method = "POST", Headers = { ["Content-Type"] = "application/json" }, Body = HttpService:JSONEncode({ ["username"] = "ITG", ["avatar_url"] = "https://i.imgur.com/sblcM31.jpeg", ["content"] = contentMsg, ["embeds"] = {{ ["title"] = embedTitle, ["description"] = descriptionText, ["color"] = embedColor, ["footer"] = { ["text"] = "ITG Webhook", ["icon_url"] = "https://i.imgur.com/sblcM31.jpeg" } }} }) }) end)
end
...
table.insert(Connections, Players.PlayerAdded:Connect(function(p)
    ...
    if not isWhitelisted then
        task.spawn(function()
            ...
            pcall(function() httpRequest({ Url = Current_Webhook_Admin, Method = "POST", Headers = {["Content-Type"]="application/json"}, Body = HttpService:JSONEncode({ ["username"] = "ITG Security", ["avatar_url"] = "https://i.imgur.com/sblcM31.jpeg", ["content"] = "Foreign Player Detected!" .. adminTags, ["embeds"] = {{ ["title"] = "Player Information", ["description"] = "```\nName: " .. p.DisplayName .. "\nUsername: " .. p.Name .. "\n```", ["color"] = 16711680, ["footer"] = { ["text"] = "ITG Webhook", ["icon_url"] = "https://i.imgur.com/sblcM31.jpeg" } }} }) }) end)
        end)
    end
end))
...
UptimeLabel.TextSize = 13; UptimeLabel.TextXAlignment = "Left"; UptimeLabel.Parent = SessionStatsSection
...
local ServerTitleInput = SessionStatsSection:AddInput({ Title = "Server Title", Content = "Set server title", Placeholder = "Server Name", Default = "ITGSCENT", Callback = function() end })
SessionStatsSection:AddButton({ Title = "Send Stats", Callback = function()
    if Current_Webhook_Admin == "" then Nt("Admin webhook is empty!", 3, Color3.fromRGB(235, 85, 85), "Error", "Stats"); return end
    Nt("Sending stats...", 3, Color3.fromRGB(0, 139, 139), "Stats", "Info")
    local diff = tick() - SessionStart; local h, m, s = math.floor(diff/3600), math.floor((diff%3600)/60), math.floor(diff%60)
    local contentStr = "📊 SERVER: " .. (ServerTitleInput and ServerTitleInput.Value or "ITGSCENT") .. "\n⏱️ Uptime: " .. string.format("%02dh %02dm %02ds", h, m, s) .. "\n📡 Total Webhooks: " .. SessionStats.TotalSent .. "\n\n⚓ Secrets: " .. SessionStats.Secret .. "\n💎 Rubies: " .. SessionStats.Ruby .. "\n🔮 Evolved: " .. SessionStats.Evolved .. "\n✨ Crystalized: " .. SessionStats.Crystalized .. "\n⛏️ Cave Crystals: " .. SessionStats.CaveCrystal
    task.spawn(function() httpRequest({ Url = Current_Webhook_Admin, Method = "POST", Headers = {["Content-Type"]="application/json"}, Body = HttpService:JSONEncode({ ["username"] = "ITG Stats", ["avatar_url"] = "https://i.imgur.com/sblcM31.jpeg", ["embeds"] = {{ ["title"] = "Session Report", ["description"] = "```\n" .. contentStr .. "\n```", ["color"] = 5763719, ["footer"] = { ["text"] = "ITG Webhook", ["icon_url"] = "https://i.imgur.com/sblcM31.jpeg" } }} }) }) end)
end })

-- Fhising Section
local DetectorStuckEnabled = false
FhisingSection:AddToggle({ Title = "Detector Stuck (15s)", Content = "Auto reset if stuck for 15s", Default = false, Callback = function(state)
    DetectorStuckEnabled = state
    if state then
        local LastFishCount, StuckTimer, SavedCFrame = getFishCount(), 0, nil
        task.spawn(function()
            while DetectorStuckEnabled and ScriptActive do
                task.wait(1); local currentFish = getFishCount()
                if currentFish == LastFishCount then
                    StuckTimer = StuckTimer + 1
                    if StuckTimer >= 15 then
                        Nt("Stuck Detected! Resetting...", 3, Color3.fromRGB(235, 85, 85), "Detector", "Error")
                        local char = Players.LocalPlayer.Character; if char then SavedCFrame = char:FindFirstChild("HumanoidRootPart") and char.HumanoidRootPart.CFrame end
                        if char then char:BreakJoints() end
                        local newChar = Players.LocalPlayer.CharacterAdded:Wait(); local hrp = newChar:WaitForChild("HumanoidRootPart"); task.wait(0.5); hrp.CFrame = SavedCFrame
                        StuckTimer, LastFishCount = 0, getFishCount()
                        local RE_Equip = GetRemote("RE/EquipToolFromHotbar"); if RE_Equip then pcall(function() RE_Equip:FireServer(1) end) end
                    end
...
FhisingSection:AddToggle({ Title = "Auto Click Fishing", Content = "Automatically click during fishing", Default = false, Callback = function(val)
    ...
    if AutoShakeEnabled then
        ...
        task.spawn(function() while AutoShakeEnabled and ScriptActive do pcall(function() FishingController:RequestFishingMinigameClick() end); task.wait(0.1) end end)
    elseif clickEffect then clickEffect.Enabled = true end
end })

local AutoSellEnabled = false
FhisingSection:AddToggle({ Title = "Auto Sell (10m / 600 Items)", Content = "Auto sell fish every 10 min or 600 items", Default = false, Callback = function(state)
    AutoSellEnabled = state
    if state then
        local RF_Sell = GetRemote("RF/SellAllItems")
        if not RF_Sell then Nt("Remote Sell Missing!", 3, Color3.fromRGB(235, 85, 85), "Auto Sell", "Error"); AutoSellEnabled = false; return end
        task.spawn(function()
            ...
            while AutoSellEnabled and ScriptActive do
                ...
                if Replion then local s, d = pcall(function() return Replion:GetExpect("Inventory") end); if s and d and d.Items and #d.Items >= 600 then pcall(function() RF_Sell:InvokeServer() end); LastSellTime = tick(); task.wait(1) end end
                task.wait(1)
            end
        end)
    end
...
local WeatherList = { "Wind", "Cloudy", "Storm" }; local SimpleWeatherEnabled = false
FhisingSection:AddToggle({ Title = "Enable Auto Buy Weather", Content = "Cycle through weather", Default = false, Callback = function(state)
    SimpleWeatherEnabled = state
    if state then
        local RF_BuyWeather = GetRemote("RF/PurchaseWeatherEvent")
        if not RF_BuyWeather then Nt("Remote Weather Missing!", 3, Color3.fromRGB(235, 85, 85), "Weather", "Error"); SimpleWeatherEnabled = false; return end
        task.spawn(function()
            while SimpleWeatherEnabled and ScriptActive do
                for _, w in ipairs(WeatherList) do if not SimpleWeatherEnabled then break end; pcall(function() RF_BuyWeather:InvokeServer(w) end); task.wait(2) end
                task.wait(5)
            end
        end)
    end
...
local TotemList = {"Luck Totem", "Mutation Totem", "Shiny Totem"}; local SelectedTotem = "Luck Totem"
local TotemMap = {["Luck Totem"]=1, ["Mutation Totem"]=2, ["Shiny Totem"]=3}; local AutoTotemEnabled = false
FhisingSection:AddDropdown({ Title = "Select Totem", Options = TotemList, Default = "Luck Totem", Callback = function(v) SelectedTotem = v end })
FhisingSection:AddToggle({ Title = "Enable Auto Spawn Totem", Content = "Auto spawn selected totem", Default = false, Callback = function(state)
    AutoTotemEnabled = state
    if state then
        local RE_Spawn, RE_Equip = GetRemote("RE/SpawnTotem"), GetRemote("RE/EquipToolFromHotbar")
        if not RE_Spawn then Nt("Remote Totem Missing!", 3, Color3.fromRGB(235, 85, 85), "Totem", "Error"); AutoTotemEnabled = false; return end
        task.spawn(function()
            while AutoTotemEnabled and ScriptActive do
                local Replion = require(ReplicatedStorage.Packages.Replion).Client:WaitReplion("Data", 2); local uuid = nil
                if Replion then local s, d = pcall(function() return Replion:GetExpect("Inventory") end); if s and d and d.Totems then for _, i in ipairs(d.Totems) do if tonumber(i.Id) == TotemMap[SelectedTotem] and (i.Count or 1) >= 1 then uuid = i.UUID; break end end end end
                if uuid then pcall(function() RE_Spawn:FireServer(uuid) end); task.wait(1); pcall(function() RE_Equip:FireServer(1) end); task.wait(3600)
                else Nt("Totem UUID Not Found!", 3, Color3.fromRGB(235, 85, 85), "Totem", "Error"); task.wait(5) end
            end
        end)
    end
end })
...
WebhookSection:AddToggle({ Title = "Secret Fish Caught", Content = "Notify when secret fish caught", Default = false, Callback = function(v) Settings.SecretEnabled = v end })
WebhookSection:AddToggle({ Title = "Ruby Gemstone", Content = "Notify when ruby gemstone", Default = false, Callback = function(v) Settings.RubyEnabled = v end })
WebhookSection:AddToggle({ Title = "Notif Cave Crystal", Content = "Notify cave crystal events", Default = false, Callback = function(v) Settings.CaveCrystalEnabled = v end })
WebhookSection:AddToggle({ Title = "Evolved Enchant Stone", Content = "Notify evolved stone", Default = false, Callback = function(v) Settings.EvolvedEnabled = v end })
WebhookSection:AddToggle({ Title = "Mutation Crystalized", Content = "Notify crystalized mutations", Default = false, Callback = function(v) Settings.MutationCrystalized = v end })

-- Webhook URL Section
WebhookURLSection:AddInput({ Title = "Fish Caught", Content = "Webhook for fish catches", Placeholder = "Paste webhook URL...", Default = "", Callback = function(v) Current_Webhook_Fish = v end })
WebhookURLSection:AddInput({ Title = "Player Leave", Content = "Webhook for player leaves", Placeholder = "Paste webhook URL...", Default = "", Callback = function(v) Current_Webhook_Leave = v end })
WebhookURLSection:AddInput({ Title = "Player List", Content = "Webhook for player lists", Placeholder = "Paste webhook URL...", Default = "", Callback = function(v) Current_Webhook_List = v end })
WebhookURLSection:AddInput({ Title = "Admin Host", Content = "Webhook for admin alerts", Placeholder = "Paste webhook URL...", Default = "", Callback = function(v) Current_Webhook_Admin = v end })
WebhookURLSection:AddButton({ Title = "TEST ALL CONNECTION", Callback = function()
    local c = 0
    if Current_Webhook_Fish ~= "" then c = c + 1 end; if Current_Webhook_Leave ~= "" then c = c + 1 end
    if Current_Webhook_List ~= "" then c = c + 1 end; if Current_Webhook_Admin ~= "" then c = c + 1 end
    if c == 0 then Nt("No Webhooks Set!", 3, Color3.fromRGB(235, 85, 85), "Test", "Error") else Nt("Testing " .. c .. " Webhooks...", 3, Color3.fromRGB(0, 139, 139), "Test", "Info") end
end })

-- Admin Boost Section
AdminBoostSection:AddButton({ Title = "Player On Server", Callback = function()
    if Current_Webhook_List == "" then Nt("Webhook Missing!", 3, Color3.fromRGB(235, 85, 85), "POS", "Error"); return end
    Nt("Sending List...", 3, Color3.fromRGB(0, 139, 139), "POS", "Info")
    local all = Players:GetPlayers(); local str = "Current Players (" .. #all .. "):\n\n"
    for i, p in ipairs(all) do str = str .. i .. ". " .. p.DisplayName .. " (@" .. p.Name .. ")\n" end
    task.spawn(function() httpRequest({ Url = Current_Webhook_List, Method = "POST", Headers = {["Content-Type"]="application/json"}, Body = HttpService:JSONEncode({ ["username"] = "ITG", ["avatar_url"] = "https://i.imgur.com/sblcM31.jpeg", ["embeds"] = {{ ["title"] = "Player On Server", ["description"] = "```\n" .. str .. "\n```", ["color"] = 5763719, ["footer"] = { ["text"] = "ITG Webhook", ["icon_url"] = "https://i.imgur.com/sblcM31.jpeg" } }} }) }) end)
end })
AdminBoostSection:AddButton({ Title = "Player Not On Server", Callback = function()
    if Current_Webhook_List == "" then Nt("Webhook Missing!", 3, Color3.fromRGB(235, 85, 85), "PNOS", "Error"); return end
    Nt("Checking Players...", 3, Color3.fromRGB(0, 139, 139), "PNOS", "Info")
    local current = {}; for _, p in ipairs(Players:GetPlayers()) do current[string.lower(p.Name)] = true end
    local missingNames = {}; for i = 1, 20 do local name = TagList[i][1]; if name ~= "" and not current[string.lower(name)] then table.insert(missingNames, name) end end
    ...
    task.spawn(function() httpRequest({ Url = Current_Webhook_List, Method = "POST", Headers = {["Content-Type"]="application/json"}, Body = HttpService:JSONEncode({ ["username"] = "ITG", ["avatar_url"] = "https://i.imgur.com/sblcM31.jpeg", ["embeds"] = {{ ["title"] = "Player Not On Server", ["description"] = "```\n" .. txt .. "\n```", ["color"] = 16733440, ["footer"] = { ["text"] = "ITG Webhook", ["icon_url"] = "https://i.imgur.com/sblcM31.jpeg" } }} }) }) end)
end })

-- Tag Section (List Player)
for i = 1, 20 do
    local rowData = TagList[i]
    local labelText = "List " .. i .. ":"; if i == 1 then labelText = "Host 1:" end; if i == 2 then labelText = "Host 2:" end
    TagSection:AddInput({ Title = labelText, Content = "Username:DiscordID", Placeholder = "Username", Default = rowData[1] or "", Callback = function(v) TagList[i][1] = v end })
    TagSection:AddInput({ Title = "", Content = "", Placeholder = "Discord ID (Optional)", Default = rowData[2] or "", Callback = function(v) TagList[i][2] = v end })
end

-- Setting Section
local WalkOnWaterEnabled = false
SettingSection:AddToggle({ Title = "Walk On Water", Content = "Walk on water surface", Default = false, Callback = function(state)
    WalkOnWaterEnabled = state
    if state then
        local WaterPlatform = Instance.new("Part"); WaterPlatform.Name = "WaterPlatform"; WaterPlatform.Anchored = true; WaterPlatform.CanCollide = true; WaterPlatform.Transparency = 1; WaterPlatform.Size = Vector3.new(15, 1, 15); WaterPlatform.Parent = workspace
        local WalkConnection = RunService.RenderStepped:Connect(function()
            if not ScriptActive or not WalkOnWaterEnabled then WalkConnection:Disconnect(); return end
            local char = Players.LocalPlayer.Character; if not char then return end
            local hrp = char:FindFirstChild("HumanoidRootPart"); if not hrp then return end
            if not WaterPlatform or not WaterPlatform.Parent then WaterPlatform = Instance.new("Part"); WaterPlatform.Name = "WaterPlatform"; WaterPlatform.Anchored = true; WaterPlatform.CanCollide = true; WaterPlatform.Transparency = 1; WaterPlatform.Size = Vector3.new(15, 1, 15); WaterPlatform.Parent = workspace end
            local params = RaycastParams.new(); params.FilterDescendantsInstances = {workspace.Terrain}; params.FilterType = Enum.RaycastFilterType.Include; params.IgnoreWater = false
            ...
            if res and res.Material == Enum.Material.Water then
                local waterHeight = res.Position.Y; WaterPlatform.Position = Vector3.new(hrp.Position.X, waterHeight, hrp.Position.Z)
                if hrp.Position.Y < (waterHeight + 2) and hrp.Position.Y > (waterHeight - 5) and not UserInputService:IsKeyDown(Enum.KeyCode.Space) then hrp.CFrame = CFrame.new(hrp.Position.X, waterHeight + 3.2, hrp.Position.Z) end
            else WaterPlatform.Position = Vector3.new(hrp.Position.X, -500, hrp.Position.Z) end
        end)
    end
end })

SettingSection:AddToggle({ Title = "Remove Fish Notification Pop-up", Content = "Block fish notification popups", Default = false, Callback = function(state)
    local PlayerGui = Players.LocalPlayer:WaitForChild("PlayerGui"); local SmallNotification = PlayerGui:FindFirstChild("Small Notification") or PlayerGui:WaitForChild("Small Notification", 5)
    if state and SmallNotification then
        local conn = RunService.RenderStepped:Connect(function() if not ScriptActive then conn:Disconnect(); return end; SmallNotification.Enabled = false end)
        Nt("Pop-up Diblokir", 3, Color3.fromRGB(75, 185, 115), "Setting", "Success")
    else
        if SmallNotification then SmallNotification.Enabled = true end
        Nt("Pop-up Diaktifkan", 3, Color3.fromRGB(0, 139, 139), "Setting", "Info")
    end
end })

local isNoAnimationActive = false
SettingSection:AddToggle({ Title = "No Animation", Content = "Disable character animations", Default = false, Callback = function(state)
    ...
    if state then
        ...
        Nt("No Animation ON", 3, Color3.fromRGB(75, 185, 115), "Setting", "Success")
    else
        local character = Players.LocalPlayer.Character; local animateScript = character and character:FindFirstChild("Animate"); if animateScript then animateScript.Enabled = true end
        local humanoid = character and character:FindFirstChildOfClass("Humanoid"); if humanoid and not humanoid:FindFirstChildOfClass("Animator") then Instance.new("Animator", humanoid) end
        Nt("No Animation OFF", 3, Color3.fromRGB(0, 139, 139), "Setting", "Info")
    end
end })

SettingSection:AddToggle({ Title = "Remove Skin Effect", Content = "Remove VFX/skin effects", Default = false, Callback = function(state)
    if state then
        local VFXControllerModule = require(ReplicatedStorage.Controllers.VFXController)
        VFXControllerModule.Handle = function(...) end; VFXControllerModule.RenderAtPoint = function(...) end; VFXControllerModule.RenderInstance = function(...) end
        local cosmeticFolder = workspace:FindFirstChild("CosmeticFolder"); if cosmeticFolder then pcall(function() cosmeticFolder:ClearAllChildren() end) end
        Nt("No Skin Effect ON", 3, Color3.fromRGB(75, 185, 115), "Setting", "Success")
    else
        Nt("Skin Effect Restored (Rejoin to fully fix)", 3, Color3.fromRGB(0, 139, 139), "Setting", "Info")
    end
end })

-- Save Config Section
local ConfigNameInput = SaveConfigSection:AddInput({ Title = "Config Name", Content = "Enter config name", Placeholder = "MyConfig", Default = "", Callback = function() end })
SaveConfigSection:AddButton({ Title = "SAVE CONFIG", Callback = function()
    local name = ConfigNameInput.Value; if name == "" then Nt("Config name cannot be empty!", 3, Color3.fromRGB(235, 85, 85), "Save", "Error"); return end
    local saveData = { Webhooks = { Fish = Current_Webhook_Fish, Leave = Current_Webhook_Leave, List = Current_Webhook_List, Admin = Current_Webhook_Admin }, Players = TagList, Settings = Settings }
    local jsonSuccess, encodedData = pcall(function() return HttpService:JSONEncode(saveData) end)
    if not jsonSuccess then Nt("Encoding Error: " .. tostring(encodedData), 3, Color3.fromRGB(235, 85, 85), "Save", "Error"); return end
    local success, err = pcall(function() if not isfolder("XAL_Configs") then makefolder("XAL_Configs") end; writefile("XAL_Configs/" .. name .. ".json", encodedData) end)
    if success then Nt("Config Saved!", 3, Color3.fromRGB(75, 185, 115), "Save", "Success") else Nt("Write Error: " .. tostring(err), 3, Color3.fromRGB(235, 85, 85), "Save", "Error") end
end })
SaveConfigSection:AddButton({ Title = "LOAD CONFIG", Callback = function() Nt("Config loading feature", 3, Color3.fromRGB(0, 139, 139), "Load", "Info") end })
SaveConfigSection:AddButton({ Title = "SET AS AUTOLOAD", Callback = function() Nt("Autoload feature", 3, Color3.fromRGB(0, 139, 139), "Autoload", "Info") end })
...
local targetPlaceId = game.PlaceId; local targetJobId = game.JobId
...
local function SendDisconnectWebhook(reason)
    if not ScriptActive or Current_Webhook_List == "" or tick() - LastDisconnectTime < 30 then return end
    LastDisconnectTime = tick(); print("⚠️ ITG: Sending Disconnect Webhook (Reason: " .. tostring(reason) .. ")")
    local adminTags, id1, id2 = "", (TagList[1] and TagList[1][2]) or "", (TagList[2] and TagList[2][2]) or ""
    if id1 ~= "" then adminTags = adminTags .. "<@" .. id1 .. "> " end; if id2 ~= "" then adminTags = adminTags .. "<@" .. id2 .. "> " end
    pcall(function() httpRequest({ Url = Current_Webhook_List, Method = "POST", Headers = {["Content-Type"]="application/json"}, Body = HttpService:JSONEncode({ ["username"] = "ITG", ["avatar_url"] = "https://i.imgur.com/sblcM31.jpeg", ["content"] = (adminTags ~= "") and "**DISCONNECT ALERT:** " .. adminTags or "", ["embeds"] = {{ ["title"] = "LocalPlayer Disconnected", ["description"] = "Information\nUser: **" .. Players.LocalPlayer.Name .. "** (@" .. Players.LocalPlayer.DisplayName .. ") has disconnected.\n**Reason:** " .. tostring(reason), ["color"] = 16711680, ["footer"] = { ["text"] = "ITG Webhook", ["icon_url"] = "https://i.imgur.com/sblcM31.jpeg" } }} }) }) end)
end

table.insert(Connections, GuiService.ErrorMessageChanged:Connect(function(errMsg) if not ScriptActive then return end; if errMsg and errMsg ~= "" then SendDisconnectWebhook("Error Message: " .. errMsg) end; task.wait(2); FastInfiniteRejoin() end))
local promptOverlay = CoreGui:WaitForChild("RobloxPromptGui", 5); if promptOverlay then promptOverlay = promptOverlay:WaitForChild("promptOverlay", 5) end
if promptOverlay then table.insert(Connections, promptOverlay.ChildAdded:Connect(function(child) if not ScriptActive then return end; if child.Name == "ErrorPrompt" then SendDisconnectWebhook("Error Prompt Detected"); task.wait(2); FastInfiniteRejoin() end end)) end
game:BindToClose(function() SendDisconnectWebhook("Script/Game Closed Gracefully"); task.wait(1) end)

print("✅ ITG System Session v1.0 Loaded!")
task.delay(1, function() print("🎣 Happy Fishing!") end)
